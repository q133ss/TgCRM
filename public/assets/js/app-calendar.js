...
document.addEventListener('DOMContentLoaded', function () {
  (function () {
    ...
    function eventClick(info) {
      ...
      if (offcanvasTitle) {
        offcanvasTitle.innerHTML = 'Обновить задачу';
      }
      btnSubmit.innerHTML = 'Обновить';
    ...
    function fetchEvents(info, successCallback) {
        return [
            {
                title: '1234', // Название события
                start: '2025-03-14', // Дата начала
                end: '2025-03-14',   // Дата окончания (опционально)
                backgroundColor: '#FF5733', // Цвет фона
                borderColor: '#FF5733'       // Цвет границы
            },
            {
                title: 'Событие 2',
                start: '2025-03-14',
                allDay: true // Все дневное событие
            }
        ];
    ...
    let calendar = new Calendar(calendarEl, {
      initialView: 'dayGridMonth',
        events: function (info, successCallback, failureCallback) {
            // info - содержит информацию о текущем представлении календаря (например, даты начала и окончания)
            ...
            $.ajax({
                ...
                success: function (response) {
                    // Передаем полученные события в FullCalendar
                    successCallback(response);
                },
                error: function (xhr, status, error) {
                    console.error('Ошибка загрузки событий:', xhr.responseJSON || error);
                    failureCallback(error); // Уведомляем FullCalendar об ошибке
                }
            });
        },
      ...
      firstDay: 1,
        buttonText: {
            today: 'Сегодня', // Кнопка "Сегодня"
            month: 'Месяц',   // Кнопка "Месяц" (dayGridMonth)
            week: 'Неделя',   // Кнопка "Неделя" (timeGridWeek)
            day: 'День',      // Кнопка "День" (timeGridDay)
            list: 'Список'    // Кнопка "Список" (listMonth)
        },
      ...
      dateClick: function (info) {
        ...
        if (offcanvasTitle) {
          offcanvasTitle.innerHTML = 'Добавить задачу';
        }
